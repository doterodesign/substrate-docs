...
From the shipped Stripe foundation (`src/brands/stripe/config.global.yaml`):

```yaml
intents:
  brand:
    hue: 277        # Stripe Indigo — #533afd
    chroma: 0.268
  neutral:
    hue: 251        # cool-tinted neutrals, pulled from Ink #0d253d
    chroma: 0.012
  danger:
    hue: 11         # Ruby #ea2261
    chroma: 0.228
  ...
  info:
    hue: 256
    chroma: 0.188
  # Stripe's signature gradient palette — custom intents, same treatment
  stripe-cyan:
    hue: 228
    chroma: 0.147
  stripe-mint:
...
<Note>
  Intent names are your identifiers, so they round-trip verbatim — the kebab-case-to-camelCase normalization described in the [config overview](/brand-config/overview) does not touch the direct children of `intents`. `stripe-cyan` stays `stripe-cyan`.
</Note>
...
What does exist is a convention. Across the shipped brands, these names recur and carry consistent meaning:
...
<Warning>
  The destructive intent is `danger`, not `error`, and the identity intent is `brand`, not `primary`. Those two names are the most common source of a config that loads but produces nothing where you expected it.
</Warning>

Brands lean on custom intents heavily. Delta declares `delta-navy` and `delta-burgundy`; SkyMiles adds `medallion-gold`, `medallion-silver`, `medallion-platinum`, `skymiles-navy`, and `tier-accent`; Magic Patterns adds `creation`, `teal`, `magic-blue`, and `magic-purple`.
...
These three grammar keys keep their kebab spelling everywhere, because the kernel's track evaluator consumes them verbatim. Scheme tracks are a supported capability that no shipped brand currently uses — all six brand families author constant hue and chroma per intent.
...
The generated brand CSS registers these with `@property` so they animate and inherit predictably. From `generated/brands/delta/skymiles/css/brand.gen.css`:

```css
@property --ucs-delta-navy-hue {
  syntax: "<number>";
  initial-value: 0;
  inherits: true;
}

@property --ucs-delta-navy-fg-l {
  syntax: "<number>";
  initial-value: 0;
  inherits: true;
}
...
Alongside the live primitives, the pipeline bakes a resolved surface/text/border triptych per intent per mode, as the no-JavaScript floor. From `generated/brands/delta/booking/css/tokens.light.gen.css`:
...
Selectors are `[data-brand="delta-skymiles"]` for brand scoping and `[data-mode~="brand"]` for intent selection on an element carrying `data-ucs`. Substrate emits no theme attribute and no per-color variable family of its own — the `--ucs-*` primitives above are the whole vocabulary.
