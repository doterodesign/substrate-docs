...
One field. All six shipped brands use the same value:
...
Most shipped brands use exactly that range; Magic Patterns opens the lower bound slightly to `0.75`.
...
But the names are the brand's to choose, and Magic Patterns proves it. It serves two contexts from one brand — a marketing surface and a product surface — and names its density levels after them rather than after their tightness:
...
`discovery` is the roomy marketing setting and `workspace` the tight product one. Same continuum, same factors as the conventional trio — different vocabulary, chosen because it matches how that team talks about their surfaces.
