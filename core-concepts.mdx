...
Colors in Substrate are defined as *intents* rather than hex values. An intent describes a color's perceptual character — its hue and chroma — without fixing its lightness. Lightness is computed by the contrast solver at every point on the scheme axis. Intents live in an open `intents:` map in the brand YAML; only `brand` and `neutral` are required, and a brand may declare any custom intents it needs. From the shipped Stripe config (`src/brands/stripe/config.global.yaml`):
...
Letting chroma rise toward the dark end is a common technique for keeping a color's perceived character consistent across luminance extremes, where colors tend to read as less saturated. Note that scheme tracks are an available capability, but no shipped brand currently uses one — the six shipped brand families all author constant hue/chroma per intent.
...
Substrate is designed to support multi-brand systems where many brands share the same mathematical model. Inheritance is structural: a brand family directory contains a shared `config.global.yaml`, and each sub-brand directory contains a `config.yaml` holding **only its deltas**. From the shipped Delta family (`src/brands/delta/skymiles/config.yaml`):
...
typography:
  heading-family: "'Whitney Condensed', Whitney, Verdana, system-ui, sans-serif"
```

The two layers are combined by a **recursive deep merge** — partial objects merge field-by-field, so SkyMiles can override just `intents.brand.chroma` and inherit the hue. Setting a key to `null` deletes it from the merged result; arrays replace wholesale. Deltas-only authoring isn't just a convention: the validator flags any sub-brand value that merely repeats its parent's. The merged brand gets the slug `delta-skymiles` (parent–child), which is what appears in the runtime's `data-brand` attribute.
...
<Tabs>
  <Tab title="Web (CSS)">
    ...
    And per-mode baked token files provide the no-JS floor — from `generated/brands/delta/booking/css/tokens.light.gen.css`:
  ...
  <Tab title="iOS (Swift)">
    Per-mode resolved token sets — separate files per mode (`system.light.gen.swift`, `system.dark.gen.swift`, …), each intent a surface/text/border triple in Display P3. From `generated/brands/magic-patterns/swift/system.light.gen.swift`:
  ...
  <Tab title="Android (Kotlin)">
    The Compose output mirrors Swift: per-mode files (`system.light.gen.kt`, `system.dark.gen.kt`, …) each declaring the same top-level object, so an app includes exactly one per build variant. From `generated/brands/magic-patterns/compose/system.light.gen.kt`:
