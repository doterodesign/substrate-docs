...
Alongside the runtime path, the pipeline bakes static per-mode files for the moments before JavaScript runs and for native platforms. These hold resolved hex values rather than primitives — from `generated/brands/magic-patterns/css/tokens.light.gen.css`:
