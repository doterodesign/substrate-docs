...
```
src/brands/delta/config.global.yaml      →  parent (shared foundation)
src/brands/delta/skymiles/config.yaml    →  child (deltas only)
```

Later layers win, per leaf. The merged result is stamped with the family slug `{parent}-{sub}` — `delta-skymiles` — which is what appears in generated CSS as `[data-brand="delta-skymiles"]`.
...
The shipped SkyMiles config is the canonical demonstration. Its parent declares the brand intent in full:

```yaml
# src/brands/delta/config.global.yaml
intents:
  brand:
    hue: 15
    chroma: 0.216
...
```yaml
# src/brands/delta/skymiles/config.yaml
...
The merged `intents.brand` is `{ hue: 15, chroma: 0.220 }`. SkyMiles never restates the hue. Likewise, its `typography` block overrides only `heading-family` — `body-family`, `base-font-size`, `scale-ratio`, `fluid`, and `density` all come from the parent untouched, and `medallion-gold` is simply added to the inherited intent roster.

Delta Cargo shows the same merge at three different depths at once. It overrides `intents.brand` entirely (both fields), `intents.neutral.chroma` alone, `typography.scale-ratio` alone, `motion.duration-base` alone (keeping the family's `linear` easing), and inside `presets` it adjusts individual mode levels' `scheme` values while leaving each level's `contrast-factor` inherited.
