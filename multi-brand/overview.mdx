...
description: "Six brand families ship in one Substrate engine. Brands are directories, discovery is a filesystem scan, and every brand inherits the same solver guarantees from its first generation."
---
...
Brands live in directories under `src/brands/`. There are two shapes, and the directory contents decide which one applies.
...
│   ├── premium/config.yaml
│   └── skymiles/config.yaml
├── stripe/
│   ├── config.global.yaml
│   └── marketing/config.yaml
└── magic-patterns/             # flat brand
    └── config.yaml
```

A directory containing `config.global.yaml` is a **family**: each subdirectory holding a `config.yaml` becomes its own brand, with the merged slug `{parent}-{sub}` — `delta-skymiles`, `delta-booking`. A directory containing only `config.yaml` is a **flat brand**, and its slug is the directory name.

Six brand families ship in the engine: Delta, Descript, DraftKings, Google, Stripe, and Magic Patterns.
...
<Warning>
  Substrate has no build verb and no per-brand build flag — you regenerate the corpus, not a single brand. The `substrate` CLI is a separate consumer-side onboarding tool whose verbs are `init`, `add`, `upgrade`, `adopt`, `setup`, and `artifact`.
</Warning>

## Per-brand output

Output lands under `generated/brands/<family>/<sub>/`, one subdirectory per target. For `delta-skymiles`:

```
generated/brands/delta/skymiles/
...
A family exists so that shared decisions live in one file. Updating `src/brands/delta/config.global.yaml` propagates to all five Delta sub-brands on the next generation; adding an intent there gives every sub-brand that intent automatically. A sub-brand's own config stays small — SkyMiles is roughly thirty lines of genuine divergence over a shared foundation — and the validator actively flags any line in it that merely restates what it already inherits.
