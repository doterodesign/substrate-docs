...
Every per-mode file declares the same two top-level types: a `SubstrateSystemTokenSet` struct of exactly three colors, and a `SubstrateSystemTokens` enum of intent roles. This is real output from `generated/brands/magic-patterns/swift/system.light.gen.swift`:
