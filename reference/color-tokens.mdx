...
For the no-JS floor and native platforms, the pipeline also bakes resolved values per mode into `generated/brands/<brand>/css/tokens.{light,dark,highContrast,darkHighContrast}.gen.css`. Each intent resolves to a **surface / text / border** triple, as hex. From `generated/brands/delta/booking/css/tokens.light.gen.css`:
...
Elements opt into the cascade with `data-ucs`, and select an intent with the `data-mode` role attribute (a space-separated token list, so state tokens combine):

```html
<div data-ucs data-mode="brand">Brand surface</div>
<span data-ucs data-mode="danger">Destructive action</span>
```
